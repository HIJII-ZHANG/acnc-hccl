...
HcclResult CollAllReduceRingFor91093Executor::KernelRunIntraServerReduceScatter(const OpParam &param, ExecMem &execMem, std::vector<std::vector<Slice> > &multRingsSliceZero, std::vector<Slice> &dataSegsSlice)
{
...
HcclResult CollAllReduceRingFor91093Executor::KernelRunInterServerAllReduce(const OpParam &param, ExecMem &execMem)
{   
    u32 perDataSize = 0;
    ...
    // 根据数据量计算每个环上数据的偏移和大小
    CHK_RET(AlgTemplateBase::PrepareSliceData(param.DataDes.count, perDataSize, sliceNum, 0, dataSegsSlice));
    ...
    u32 level0ServerIndex = topoAttr_.userRank % sliceNum;
    DeviceMem dstMem;
    DeviceMem srcMem;
    u64 inputMemSize = execMem.inputMem.size();
    u64 hdSize = inputMemSize < dataSegsSlice[level0ServerIndex].size ? inputMemSize : dataSegsSlice[level0ServerIndex].size;
    u64 hdCount = hdSize / perDataSize;

    u32 segmentIdx = level0ServerIndex;
    u32 commIndex =level0ServerIndex;

    const u32 ZERO_OFFSET = 0;
    // copy data from user_in -> ccl_in
    dstMem = execMem.inputMem.range(ZERO_OFFSET, hdSize);
    ...
    if (topoAttr_.superPodNum <= 1 || isSelectAHC) {
        DeviceMem allreduceInput = execMem.inputMem.range(ZERO_OFFSET, hdSize);
        CHK_SMART_PTR_NULL(allreduceInput);
        DeviceMem allreduceOutput = execMem.outputMem.range(ZERO_OFFSET, hdSize);;
        ...
        CHK_RET(level1TempAlg->Prepare(
            ...
            std::vector<Slice>(0), ZERO_OFFSET));
        ...
        srcMem = execMem.outputMem.range(ZERO_OFFSET, hdSize);
        ...
        u64 level1OffsetUser = dataSegsSlice[segmentIdx].offset;
        u64 level1Offset = ZERO_OFFSET;  // 在ccl buffer里面顶格存放
...
HcclResult CollAllReduceRingFor91093Executor::KernelRunIntraServerAllGather(const OpParam &param, ExecMem &execMem, std::vector<std::vector<Slice> > &multRingsSliceZero, u64 &hdCount)
{    
