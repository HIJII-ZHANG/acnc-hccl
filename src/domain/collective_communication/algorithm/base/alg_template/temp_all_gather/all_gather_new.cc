...
namespace hccl {
AllGatherNew::AllGatherNew(const HcclDispatcher dispatcher)
    : AlgTemplateBase(dispatcher)
{}

AllGatherNew::~AllGatherNew() {}

HcclResult AllGatherNew::Prepare(std::vector<Stream> &meshStreams,
...
HcclResult AllGatherNew::Tx(const LINK &link, const Slice &txSlice, const Slice &dstSlice, Stream stream)
{
...
HcclResult AllGatherNew::Rx(const LINK &link, const Slice &srcSlice, const Slice &rxSlice, Stream stream)
{
...
HcclResult AllGatherNew::RunAllGather(const std::vector<LINK> &links, const std::vector<Slice> &outputSlices,
...
HcclResult AllGatherNew::RunAsync(const u32 rank, const u32 rankSize, const std::vector<LINK> &links)
{
    ...
    HCCL_INFO("AllGatherNew run: rank[%u] totalrank[%u] inputMem[%p] outputMem[%p] count[%llu]", 
